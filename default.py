...
__settings__ = xbmcaddon.Addon(id='plugin.video.MyLiveTV')
...
# setup cache dir
__scriptname__  = 'MyLiveTV'
__scriptid__ = "plugin.video.MyLiveTV"
...
def getSources():
        ...
        if os.path.exists(source_file)==False:
            xbmc.executebuiltin("XBMC.Notification(MyLiveTV,Choose type source and then select Add Source.,15000,"+icon+")")
...
def addSource(url=None):
        ...
        xbmc.executebuiltin("XBMC.Notification(MyLiveTV,New TV added.,5000,"+icon+")")
...
def getCommunitySources():
        req = urllib2.Request('http://code.google.com/p/mytv-channels-list/source/browse/trunk/')
        ...
        for i in files:
            name = i('a')[0]['href']
            url = 'http://code.google.com/p/mytv-channels-list/source/browse/trunk/'+name
...
def getCommunitySources():
        ...
        response.close()
        soup = BeautifulSoup(link, convertEntities=BeautifulSoup.HTML_ENTITIES)
        files = re.compile('<a href="(.+?)">MyLiveTV.xml</a>').findall
        name = 'MyLiveTV.xml'
        url = 'http://code.google.com/p/mytv-channels-list/source/browse/trunk/'+name
        addDir(name,url,11,icon,fanart,'','','',False)
...
def getUpdate():
        ...
        if 'smb:' in save_location:
                ...
                try:
                    ...
                    return
        xbmc.executebuiltin("XBMC.Notification(MyLiveTV,MyLiveTV Updated,5000,"+icon+")")
...
if __settings__.getSetting('MyLiveTV') == "true":
    if __settings__.getSetting('save_location') == "":
        xbmc.executebuiltin("XBMC.Notification('MyLiveTV','Choose a location to save files and select OK to save.',15000,"+icon+")")
        __settings__.openSettings()
    else:
        try:
            checkForUpdate()
...
def getSoup(url):
        ...
        if url.startswith('http://'):
            try:
                ...
                if hasattr(e, 'code'):
                    print 'We failed with error code - %s.' % e.code
                    xbmc.executebuiltin("XBMC.Notification(MyLiveTV,We failed with error code - "+str(e.code)+",10000,"+icon+")")
                elif hasattr(e, 'reason'):
                    print 'We failed to reach a server.'
                    print 'Reason: ', e.reason
                    xbmc.executebuiltin("XBMC.Notification(MyLiveTV,We failed to reach a server. - "+str(e.reason)+",10000,"+icon+")")
        else:
...
def addLink(url,name,iconimage,fanart,description,genre,date,showcontext=True):
        ...
        if showcontext:
            try:
                if name in FAV:
                    contextMenu = [('Remove from MyLiveTV Favorites','XBMC.Container.Update(%s?mode=6&name=%s)' %(sys.argv[0], urllib.quote_plus(name)))]
                else:
                    contextMenu = [('Add to MyLiveTV Favorites','XBMC.Container.Update(%s?mode=5&name=%s&url=%s&iconimage=%s&fanart=%s)' %(sys.argv[0], urllib.quote_plus(name), urllib.quote_plus(url), urllib.quote_plus(iconimage), urllib.quote_plus(fanart)))]
            except:
                contextMenu = [('Add to MyLiveTV Favorites','XBMC.Container.Update(%s?mode=5&name=%s&url=%s&iconimage=%s&fanart=%s)' %(sys.argv[0], urllib.quote_plus(name), urllib.quote_plus(url), urllib.quote_plus(iconimage), urllib.quote_plus(fanart)))]
